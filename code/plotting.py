...
def plot_pca_components_traces(pca_motion_energy, x_trace_seconds, component_indices=[0, 1, 2], axes=None):
    ...
    for i, ax in enumerate(axes):
        ...
        ax.set_title(f'PCA {component_indices[i] + 1}', fontsize=14)
        ax.tick_params(axis='both', which='major', labelsize=14)
        ax.legend(['smoothed \ntrace'], fontsize = 10)
        ax.set_xlim([0, x_trace_seconds[-1]])
        ax.set_ylim([np.nanmean(smoothed_trace)-np.nanstd(smoothed_trace)*5, np.nanmean(smoothed_trace)+np.nanstd(smoothed_trace)*5])
        ax.grid(True)
